...
- `@media (max-width:899px){ .hero-carousel--solo{ min-height:400px; } }` &mdash; tinggi carousel di layar sempit dikembalikan lebih pendek (sebelumnya 460px lalu 560px di &lt;640px, tinggi ekstra itu memang khusus untuk memberi ruang kartu yang menumpuk vertikal, lihat bag. 6.2) karena tidak ada lagi elemen yang perlu ditampung di bawah teks.
...
**Verifikasi:** dijalankan lewat server statis lokal (`python3 -m http.server`), `index.html` &amp; `index-card.html` sama-sama dicek HTTP 200. Tidak ada browser/Chrome DevTools tersedia di environment ini untuk screenshot visual (ekstensi Claude in Chrome ditolak user sesi ini, tidak ada Chromium/Node terpasang) &mdash; diverifikasi lewat pemeriksaan kode: `diff` `index.html` vs `index-card.html` dicek cuma beda di blok kartu &amp; 1 class tambahan (tidak ada perubahan tak sengaja di bagian lain halaman), override CSS dicek urutan &amp; spesifisitasnya menang di semua breakpoint, dan `assets/main.js` dicek listener form pencarian (`heroSearchForm`/`heroSearchInput`/`heroSearchResults`) sudah dibungkus pengecekan `if (heroSearchForm && ...)` sehingga aman dihilangkan dari `index.html` tanpa error JS.
