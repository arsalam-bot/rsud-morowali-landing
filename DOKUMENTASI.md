...
**Title bar (bag. 6r) dirapikan di ke-29 halaman**: Beranda jadi cuma "RSUD Morowali", halaman lain "Nama Halaman - RSUD Morowali" (tanda hubung biasa, bukan em dash lagi).
**Bug ditemukan &amp; diperbaiki (bag. 6q): grid Poliklinik/Ruang Rawat Inap di `layanan.html` overflow ke kanan di layar mobile asli** (2 kolom `1fr` tidak menyusut merata gara-gara nama poliklinik panjang, plus kata tanpa spasi seperti "Umum/Konsultasi" kepotong di tengah kata) &mdash; diperbaiki (`min-width:0` di kartu, `overflow-wrap:anywhere` di judul, grid Poliklinik/Rawat Inap jadi 1 kolom penuh di &lt;521px). Dicek langsung pakai Playwright headless yang sudah terpasang di environment ini (bag. 6p), bukan cuma baca kode.
...
## 6q. Bug: grid Poliklinik/Ruang Rawat Inap overflow ke kanan di mobile (`layanan.html`) &mdash; DIPERBAIKI

User melaporkan (dengan screenshot DevTools mobile emulation) kartu poliklinik "melewati layar ke kanan" di mode mobile, sementara kartu lain normal. Diperiksa langsung pakai Playwright headless Chromium yang sudah terpasang di environment ini sejak bug hero mobile (bag. 6p) &mdash; server statis lokal + `page.evaluate`/`getBoundingClientRect` untuk memeriksa lebar kolom grid yang sebenarnya, bukan cuma baca kode.

**Akar masalah #1 (utama, penyebab overflow layar):** `.service-grid{ grid-template-columns:repeat(2,1fr); }` dipakai di lebar &lt;720px untuk semua grid layanan, termasuk `.service-grid--poli` (Poliklinik &amp; Ruang Rawat Inap, 20 &amp; 7 kartu). Grid item secara default punya `min-width:auto`, bukan `0` &mdash; artinya track `1fr` tidak bisa menyusut di bawah lebar konten alaminya. Karena nama poliklinik di tiap kolom beda panjang (kolom ganjil kebagian nama lebih panjang seperti "Poliklinik Penyakit Dalam", "Poliklinik Jantung dan Pembuluh Darah"), 2 kolom yang seharusnya sama rata malah dihitung timpang (`220px` vs `177px`, dicek langsung lewat `getComputedStyle`), totalnya melebihi lebar layar &rarr; `document.documentElement.scrollWidth` 431px di viewport 390px, kolom kanan kepotong/lewat layar.

**Akar masalah #2 (residual, ditemukan setelah #1 diperbaiki):** beberapa nama poliklinik mengandung kata tanpa spasi yang tidak bisa dipenggal browser di tempat wajar, mis. "Umum/Konsultasi" (garis miring bukan titik potong kata default) dan "Periodonsia"/"Endodonsia" (1 kata panjang). Di kolom sempit, ini bikin judul kartu meluber ~35px keluar dari kartunya sendiri &mdash; tidak sampai bikin scrollbar halaman (ketutup `overflow-x:hidden` di `body`, dipasang untuk keperluan lain, bag. 6.2), tapi teksnya kepotong/hilang secara visual.

**Perbaikan (3 bagian, semua di `assets/styles.css`):**
1. `.service-card{ min-width:0; }` &mdash; supaya kolom grid `1fr` benar-benar bisa menyusut rata, konsisten dengan proporsinya, teks membungkus (wrap) di dalam kartu alih-alih mendorong lebar track. Berlaku ke semua grid layanan (Poliklinik, Rawat Inap, Layanan Penunjang, IGD), bukan cuma yang dilaporkan.
2. `.service-card h3{ overflow-wrap:anywhere; }` &mdash; kata tanpa titik potong wajar (mis. "Umum/Konsultasi") baru dipenggal kalau memang tidak ada pilihan lain, bukan default agresif yang memenggal semua kata.
3. `@media (max-width:520px){ .service-grid--poli{ grid-template-columns:1fr; } }` &mdash; di lebar HP asli (dicek 360&ndash;428px, mencakup iPhone SE sampai Pro Max &amp; kebanyakan Android), 2 kolom cuma menyisakan ~80px untuk teks judul &mdash; cukup untuk tidak overflow (perbaikan #1+#2 sudah menjamin itu), tapi nama panjang masih kepenggal di tengah kata secara tidak enak dilihat (mis. "Konsul-tasi", "Periodons-ia"). 1 kolom penuh (~350px) memberi ruang supaya wrap-nya tetap rapi 1&ndash;2 baris wajar. Dicek breakpoint-nya secara empiris (bukan tebakan): masih kepenggal di 480px, sudah rapi di 500&ndash;540px &mdash; 520px dipilih sebagai batas aman.

**Verifikasi:** discreenshot &amp; dicek `getBoundingClientRect`/`scrollWidth` di lebar 360, 390, 428 (HP asli), 480&ndash;540 (breakpoint transisi), dan 1440 (desktop) &mdash; nol elemen overflow di semua lebar mobile yang dicek, desktop (grid 4 kolom, tidak kena bug ini karena kolomnya lebih banyak/lebih sempit per-kartu jadi tidak pernah mencoba muat kata panjang dalam ruang sesempit itu) tidak berubah sama sekali. Section Ruang Rawat Inap (`.service-grid--poli` yang sama) &amp; IGD/Layanan Penunjang (`.service-grid` biasa) ikut dicek, tidak ada regresi.

---

## 6r. Format title bar (tab browser) dirapikan di ke-29 halaman &mdash; SELESAI

Permintaan langsung: title bar Beranda cuma **"RSUD Morowali"** (tanpa tagline), halaman lain formatnya **"Nama Halaman - RSUD Morowali"**.

- **`index.html`**: `<title>` disederhanakan dari "RSUD Morowali &mdash; Pelayanan Kesehatan untuk Masyarakat Morowali" jadi **"RSUD Morowali"** saja.
- **27 halaman lain**: sudah lebih dulu ikut pola "Nama Halaman [pemisah] RSUD Morowali" (tidak perlu disusun ulang), cuma pemisahnya diganti dari em dash (`&mdash;`) ke **tanda hubung biasa (`-`)** sesuai contoh user ("Layanan - RSUD Morowali"). Sekalian membetulkan pelanggaran R-02 antislop (em dash dilarang di teks apa pun) yang sebelumnya ada di seluruh title bar situs sejak awal &mdash; baru ketahuan sekarang karena filter antislop dipasang belakangan, bukan sejak title bar ini pertama dibuat.
- **Sengaja tidak disentuh:**
  - `index-card.html` (duplikat `index.html` versi sebelum revisi hero, bag. 6p) &mdash; title-nya dibiarkan seperti semula, apa adanya, karena halaman ini memang dimaksudkan sebagai salinan beku titik-waktu sebelumnya, bukan halaman yang ikut ter-update tiap ada perubahan di `index.html`.
  - `og:title`/`twitter:title` (meta Open Graph/Twitter Card, bag. 6k) &mdash; permintaan user spesifik soal "title bar" (tab browser), bukan preview link media sosial. Efeknya: preview link WhatsApp/Facebook untuk Beranda utamanya sekarang beda dari title bar (masih pakai tagline lengkap + em dash) &mdash; kalau nanti mau disamakan juga, tinggal bilang.
